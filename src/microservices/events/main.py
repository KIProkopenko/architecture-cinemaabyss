...
# Pydantic модель: ожидаем тело вида {"data": { ... }}
...
async def startup():
    ...
    producer = AIOKafkaProducer(
        ...
        # Для надёжности можно добавить retries=3, acks="all"
    )
    ...
    try:
        async for msg in consumer:
            try:
                ...
                logger.info(
                    f"[CONSUMER] ✅ Received event "
                    f"type={event_type!r} | time={event_time} | data={event_payload}"
                )
            except Exception as e:
                logger.error(f"[CONSUMER] ❌ Failed to process message: {e} | raw={msg.value}")
    finally:
        await consumer.stop()
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"✅ Events service starting on port {PORT}")
